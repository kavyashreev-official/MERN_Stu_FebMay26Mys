...
async function generateOTP(userId) {
    ...
    await OTP.findOneAndUpdate(
        ...
        {
            upsert: true, //if doc not exist then create one
...
async function main() {
    ...
    // const result = await verifyOTP("user1", userEnteredOtp);  //o/p-valid
    // const result=await verifyOTP("user1","1231");  // o/p-invalid
    // const result=await verifyOTP("user2",userEnteredOtp);  //o/p-no otp
    const result=setTimeout(async() => {
        console.log(verifyOTP("user1", userEnteredOtp));  //o/p-expired 
    });
    console.log(result);
